...
public:
    ...
    DogmNode() : nh_(), pnh_("~")
    {
        ...
        grid_map_ = std::make_unique<DynamicGridMap>(
            ...
            particle_static_vel_thresh_,
            radar_static_vel_thresh_
        );
...
private:
    ...
    void odomCb(const nav_msgs::Odometry::ConstPtr& msg)
    {
        ...
        if (show_ego_velocity_arrow_)
        {
            ...
            arrow_msg.scale.x = 0.05; // shaft diameter
            arrow_msg.scale.y = 0.1;  // head diameter
            arrow_msg.scale.z = 0.4;  // head length
    ...
    void updateLoop(const ros::TimerEvent&)
    {
        ...
        // (radar_static_penalty_strength_ removed from call)
        ...
        // 6. Calculate velocity statistics (The "Brain")
        // (particle_static_vel_thresh_ removed from call)
        grid_map_->calculateVelocityStatistics(
            max_vel_for_scaling_,
            use_ego_comp_, ego_vx_, ego_vy_
        );

        // 7. Generate new particles (The "Bootstrap")
        // (max_radar_speed_for_scaling_ re-added to call)
    ...
    void loadParams()
    {
        ...
        // This parameter is required for the bootstrap logic
        ...
        // radar_static_penalty_strength is correctly removed
    }
...
private:
    ...
    double max_radar_speed_for_scaling_; // Re-added
    ...
    // radar_static_penalty_strength_ is correctly removed
    
    // Dual Threshold Members
