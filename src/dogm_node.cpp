...
void interpolateColor(double velocity, double& r, double& g, double& b) {
    const double blue[3] = {0/255.0, 0/255.0, 255/255.0};
    ...
    if (velocity < 0.3) {
        ...
        r = (1.0 - scale) * blue[0] + scale * purple[0];
        g = (1.0 - scale) * blue[1] + scale * purple[1];
        b = (1.0 - scale) * blue[2] + scale * purple[2];
...
public:
    DogmNode() : nh_(), pnh_("~") {
        ...
        ROS_INFO_STREAM("DOGM with Radar fusion node started.");
    }
private:
    ...
    void radarCb(const sensor_msgs::PointCloud2::ConstPtr& msg) {
        ...
        points_viz.ns = "radar_velocity_colormap";
        points_viz.id = 0;
        ...
        points_viz.scale.x = 0.03;
        points_viz.scale.y = 0.03;
        points_viz.lifetime = ros::Duration(0.15);
        if(has_radar_){
    ...
    void updateLoop(const ros::TimerEvent&) {
        ...
        grid_map_->generateMeasurementGrid(last_scan_, has_radar_ ? last_radar_cloud_ : nullptr);
        has_radar_ = false;
    ...
    void loadParams() {
        ...
        pnh_.param("radar_newborn_vel_stddev", radar_newborn_vel_stddev_, 1.0);
...
private:
    ...
    double dynamic_birth_ratio_, dynamic_newborn_vel_stddev_, radar_newborn_vel_stddev_;
    ...
    // [오류 수정] 삭제되었던 멤버 변수들 다시 선언하고 오타 수정
